...
export const EmployeeEditComponenet:React.FC<IProps> = ({data, onSave}) => {
    ...
    const handleInputChange = (field: keyof IEmployeeData, value: string | number) => {
      ...
      const validation = ():boolean => {
        ...
        const validLastName = empData.firstName.trim() !== '';
        ...
        if (validFirstName && validLastName && validEmpNumber && validGenderSelection && validateSalutation) {
            return true;
        } else {
            return false;
        }
      ...
      const presistHanlder = () => {
        if (validation()) {
            createOrUpdateUserProfile(empData).then( ()=> {
                onSave();
                setEmpData(createBlankUserData());
